...
3. Нажмите **File->Export->Export Hardware...** и выберите локацию `ZynqDesign.lab6`. Убедитесь, что вы экспортируете *bitstream*, так как теперь у нас есть IP в PL. Нажмите **OK**.
    ...
    ![Добавить битстрим](./resources/lab6/Include%20Bitstream_2.png)

4. Запустите *Vitis IDE*. Установите в качестве Workspace ~~на только что созданную локацию **ZynqDesign.lab6/**~~ `С:\Users\<USER NAME>\ZynqDesign.lab6\ZynqDesign.vitis`.

    Создание нового рабочего пространства означает, что наши прошлые созданные приложения и платформы не будут отображаться. Однако контролирование рабочего пространства и то, что туда импортируется и обновляется, очень важно. Сейчас мы пересоздадим *Platform Component* и импортируем туда ранее созданные приложения.

5. Создайте новый *Platform Component*, как это делалось в предыдущих работах, используя созданный в этой работе **.xsa** файл.  

6. Откройте окно импорта, выбрав в меню **File -> Import...**. В открывшимся окне выберите архив, полученный в конце третьей лабораторной работы. Нажмите **Next**.

    ![Импорт архива](./resources/lab6/Import%20Archive.png)

7. Укажите, какие компоненты вы хотите импортировать. Нажмите **Next** и **Finish**. 

    ![Импорт компонент](./resources/lab6/Import%20Components.png)


8. Импортированные приложения будут добавлены в текущую рабочую область. Выполните их сборку и загрузку для проверки работоспособности. 

    ![Результат запуска Hello World](./resources/lab2/Lab1_scrin.png)

    ![Результаты запуска MemTest](./resources/lab3/Memory%20Test%20Results.png)

    ![Результаты запуска PeriphTest](./resources/lab3/Peripheral%20Test%20Results.png)

9. Для тестирования BRAM уже есть нужный исходный код. Однако нужно создать *Application Component*. Создайте новый компонент из примера **Empty Application**. Укажите `BRAM_DMA_Test` в качестве имени.

    ![Создание пустого проекта](./resources/lab6/Create%20Empty%20App.png)

10. Скачайте файл `dma_test.c`, приложенный к описанию данной работы. 

11. В окне *Vitis Explorer* раскройте **BRAM_DMA_Test/Source**, нажмите правой кнопкой мыши на **src** и выберите **Import --> Files...**

    ![Импортировать исходники](./resources/lab6/Import%20code%20source.png)

12. В открывшемся окне выберете скачанный ранее файл `dma_test.c`.

13. Соберите проекты платформы и приложения.

14. Теперь, поскольку для наших целей мы используем PL, оно должно быть запрограммировано. Убедитесь, что в конфигурации запуска установлена галочка напротив **Program Device**.
