...
Этот эксперимент покажет, как создать IP-проект, определить AXI-интерфейс и зарегистрировать настройки для IP.

---

### **Обобщенная инструкция:**

Создайте новый периферийный AXI блок.
...
4. Мы будем создавать новую AXI-периферию, поэтому отметьте пункт **Create a New AXI4 peripheral**. Нажмите кнопку **Next**.
    ![Выбор создания новой AXI периферии](./resources/lab7/Create%20a%20New%20AXI%20Peripheral.png)

5. Введите в соответствующие поля следующую информацию:
    * Name: `PWM_w_Int`
    * Version: `1.0`
...
2. Выберите блок в каталоге IP, введя в поиске `PWM`. Нажмите правой кнопкой мыши и выберите **Edit in IP Packager**.

    ![Отредактировать IP](./resources/lab7/Edit%20IP%20Project.png)

3. Система создаст временный проект для редактирования IP блока. По умолчанию проект будет расположен в папке `ZynqDesign/ZynqDesign.tmp`. Мы принимаем настройки имени и расположения проекта по-умолчанию и нажимаем **OK**.
...
2. Откроется панель, которая поможет нам пошагово упаковать IP для IP-каталога. В первом окне *Identification* имеется много параметров, которые мы вводили при создании проекта. **Проверьте эти параметры**. Обратите внимание на версию IP в этом окне. Если в проекте будут совершены изменения, значение версии может быть изменено здесь. Можете поменять параметры касающиеся *Vendor* и *Company*
...
7. Скопируйте файл `PWM_IP_Core_Datasheet.pdf` из  из папки `/Support_documents` в `U:\ZynqHW\ip_repo\PWM_w_Int_1_0\doc`, предварительно ее создав. Теперь кликните правой кнопкой мыши по **Product Guide** и выберите **Add Files**.
    
    ![Добавление файла документации](./resources/lab7/Add%20Product%20Guide.png)
...
<!-- 13. Раскройте **Clocks and Reset Signals->S00_AXI_CLK**. Нажмите правой кнопкой мыши на **S00_AXI_CLK** и выберите **Edit Interface...**  Во вкладке *Parameters* выберите пункт **Requires User Settings**. В нем выберите **FREQ_HZ**. Нажмите на стрелочку для переноса параметра. После добавления параметра нажмите **ОК**
...
18. По окончанию *Vivado* предложит закрыть проект. Нажмите **Yes**. В этот момент временный проект, который создавался в папке `U:\ZynqLabs\ZynqDesign\ZynqDesign.tmp` будет удален. 
...
7. Выберите **Interrupts** в навигаторе страниц. Установите флажок **Fabric Interrupts**, затем разверните его. Разверните раздел **PL-PS Interrupt Ports** и установите флажок рядом с IRQ_F2P[15:0]. F2P — это соединение между ПЛИС и PS. После завершения нажмите **«ОК»**.

    ![Добавление прерываний](./resources/lab7/Add%20PL-PS%20Interrupts.png)

8. Подключите выход Interrupt_Out от PWM IP к IRQ_F2P[0:0] на Zynq PS, щелкнув по любому из контактов, создав линию, а затем щелкнув по другому контакту.
...
9. Снова зайдите в раздел «Добавить IP» и найдите ILA:

    ![Добавление ILA](./resources/lab7/Add%20ILA.png)

    Добавьте его в дизайн двойным щелчком.

10. Сделайте двойной щелчок по **ILA**, чтобы открыть его настройки для редактирования. 

11. Установите Monitor Type на **Native** и задайте количество датчиков равным 4. Затем щелкните вкладку **Probe_Ports**.

    ![Настройка ILA](./resources/lab7/Customize%20ILA%20IP.png)

12. Измените значение Probes Widths следующим образом и нажмите **ОК**:
...
15. Подключите *LED* к *PROBE0*, *PWM_Counter* к *PROBE1*, *DutyCycle* к *PROBE2*, а *Interrupt_out* к *PROBE3*. Подключите вход анализатора *clk* к выходу процессора *FCLK_CLK0*.
