...
Vivado имеет большое количество готоых настроек для отладочных платформ, такие как Avnet ZedBoard или MicroZed. Однако в данной работе проект Zynq будет создан с нуля с выполнением всех настроек вручную.

---

### **Обобщенная инструкция:**

Запустите Vivado 2026.1 и создайте проект в заданной директории:
...
1. Запустите Vivado 2026.1 с помощью иконки на рабочем столе.

2. В поле **Quick Start** выберите **New Project**.

    ![Создание проекта Vivado](./resources/lab1/Vivado%20Project%20Creation.png)

3. Откроется мастер создания проектов.

4. В окне *Project Name* Введите в поле *Project Name* имя проекта `ZynqDesign`. В поле *Project Location* введите директорию, в которой будет располагаться проект. Например, `U:\ZynqLabs`. Убедитесь, что включено создание поддиректории проекта (чекбокс *Create project subdirectory*).
...
7. Важно: в окне *Add Sources* в поле *Target Language* выберите **Verilog**. Нажмите кнопку **Next** два раза.
    
    ![Настройка добавления исходников](./resources/lab1/Add%20Sources%20Settings.png)

8. Далее в окне *Default Part* необходимо выбрать целевое устройство. Для этого можно выбрать определенные части устройства или плату целиком. Несмотря на то, что здесь мы можем выбрать плату *ZedBoard*, мы не будем делать этого, так как для данной лабораторной работы целью является создание своей собственной сборки. Вберете опцию **Parts**, активируйте фильтры и задайте следующие опции фильтра:
...
10. В результате появится интерфейс Vivado Design. Изучите доступные функции и ответьте на вопросы ниже.
...
3. В Vivado появится область с диаграммой нашей *Z_system*. Выберите иконку **Add IP** с верхней панели или из области диаграммы.
...
7. Нажмите кнопку **Save Block Design** в верхней панели Vivado bли используйте сочетание клавиш **Ctrl+S** для сохранения Block Design.

8. На данный момент микропроцессорная система PS ZYNQ7 полностью не сконфигурирована. Чтобы настроить нашу систему два раза нажмите на IP-блок **ZYNQ7 Processing System** . Большинство IP может быть настроено таким образом.
