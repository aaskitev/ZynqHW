...
3. Дважды кликните на **ZYNQ7 Processing System** для изменения настроек процессорного ядра.
...
1. В боковой панели *Page Navigator* откройте **Clock Configuration**. Перейдите во вкладку **Advanced Clocking** и раскройте все списки клоков, нажав **Expand All**.
...
3. Перейдите назад во вкладку **Basic Clocking** и установите *ENET0* в **1000 Mbps**.
...
6. PS также умеет управлять 4-мя тактовыми сигналами, которые передаются в ПЛИС. Включите и установите в следующие значения:
...
12. *Block Design* это всего лишь графическая репрезентация некоторого HDL-кода, который будет сгенерирован нашей системой. Код и файлы, сгенерированные *Block Design* являются **Output Products**.Когда мы в первый раз сгенерировали битстрим, система автоматически сгенерировала для нас *Output Products*. Теперь мы модифицировали *Block Design*. Вообще система должна обнаружить изменения нашего *Block Design* и сгенерировать наши *Output Products* заново. Однако для большей безопасности мы вручную удалим файлы и сгенерируем *Output Products*.
...
16. Откройте *Implemented design*. Если эта выбор с этой опцией не появился, нажмите **Cancel**, откройте *implemented design*  боковой панели и выберите **Reload**.
...
Экспортируйте в *Vitis*. Сгенерируйте Memory и Peripheral test application.
...
2. Экспортируйте в Vitis, как вы это делали в 2 лабораторной работе. Выберите **File -> Export -> Export Hardware...**
...
8. Создайте приложение `Memory_Tester` из примера *Memory Tests* по аналогии с Hello World:

    * Выберете пример **Memory Tests**
    * Введите *Component Name* `Memory_Tester`.
    * Выберите ту же платформу `Z_System_platform`, что и для Hello World.
    * Выполните сборку проекта, выбрав нужный компонент в выпадающем меню.
...
9. Аналогичную процедуру проведите для **Peripheral Tests**.
    ...
    ![Результаты запуска](./resources/lab3/Peripheral%20Test%20Results.png)

10. Экспортируем приложения для последующего использования. 
 
    Для архивации приложений выберите **File -> Export**.
    
    В открывшемся окне установите чекбосы на против всех программных проектов. Задайте имя архива. Нажмите **Next** и **Finish**.

    ![Экспорт приложений](./resources/lab3/Export%20App%20Component.png)

    Убедитесь, что архив был успешно создан. Он потребуется для дальнейшей работы. 
